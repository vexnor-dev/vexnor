...
import { mssqlSchema, mssqlView, varchar, nvarchar, int } from "drizzle-orm/mssql-core";
import { sql, row, param, SqlTable } from "valnor";
...
const accountOrderSummaryDrizzle = mySchema
   .view("account_order_summary", {
      ...
      firstName: nvarchar("first_name", { length: 100 }),
      orderCount: int("order_count"),
   })
   .existing();

const noSchemaDrizzle = mssqlView("account_order_summary", {
   accountId: varchar("account_id", { length: 36 }),
   email: varchar("email", { length: 255 }),
}).existing();

describe("fromDrizzleView (mssql) — metadata", () => {
   test("throws if called with a builder instead of .existing()", () => {
      const builder = mssqlSchema("valnor_test").view("test", { id: varchar("id", { length: 36 }) });
   ...
   test("fromDrizzleView — with schema", () => {
      expect(fromDrizzleView(accountOrderSummaryDrizzle)).toMatchInlineSnapshot(`
        SqlTable {
          ...
          "dialect": "tsql",
          "format": null,
          "id": "SqlTable#3(valnor_test.account_order_summary)",
          "pk": [],
          "tableInfo": {
            "name": "account_order_summary",
            "schema": "valnor_test",
   ...
   test("fromDrizzleView — schema override", () => {
      expect(fromDrizzleView(noSchemaDrizzle, "dbo").tableInfo.schema).toMatchInlineSnapshot(`"dbo"`);
   });
   ...
   test("dialect", () => {
      expect(fromDrizzleView(accountOrderSummaryDrizzle).dialect).toMatchInlineSnapshot(`"tsql"`);
   });
});

describe("fromDrizzleView (mssql) — SQL generation", () => {
   ...
   test("SELECT all columns", () => {
      expect(sql`SELECT ${row(View.$$)} FROM ${View}`.getSql({}).text).toMatchInlineSnapshot(`
        ...
        FROM
          "valnor_test"."account_order_summary" AS "aos_1" /* </query_0> */"
      `);
   });
   ...
   test("SELECT with WHERE param", () => {
      ...
      expect(
        ...
        SELECT
          ...
          "aos_1"."order_count" AS "orderCount"
        FROM
          "valnor_test"."account_order_summary" AS "aos_1"
        WHERE
          "aos_1"."email" = @param_0 /* </query_0> */"
      `);
   });
});
