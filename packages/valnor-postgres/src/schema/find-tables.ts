...
const PrimaryKeys = sql`
   select ${row(KeyColumnUsage.$table_name, KeyColumnUsage.$table_schema)},
          ${val`json_agg(${KeyColumnUsage.$column_name} order by ${KeyColumnUsage.$ordinal_position})`.as(`pk_columns`)}
   from ${KeyColumnUsage}
...
export const findTables = sql`
   with ${TableColumns},
        ${PrimaryKeys}
   select ${row(TableColumns.$table_name, TableColumns.$table_schema)}, ${PrimaryKeys.$pk_columns(`primary_key`)},
