...
describe("SqlSet", () => {
   ...
   test("produces no output for null param", () => {
      ...
      const result = query.getSql({
         // @ts-expect-error testing null set at runtime
         params: { set: null, accountId: "123" },
         options: { dialect: "sql", format: false },
      });
   ...
   test("produces no output for undefined param", () => {
      const query = sql`
         UPDATE ${Account}
         ${set(Account)}
         WHERE ${Account.$accountId} = ${param<UpdateParams>("accountId")}
      ...
      const result = query.getSql({
         params: { set: undefined as unknown as Record<string, unknown>, accountId: "123" },
         options: { dialect: "sql", format: false },
      });
