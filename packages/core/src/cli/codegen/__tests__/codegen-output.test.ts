...
describe("codegenCommand output", () => {
   ...
   test("generates correct output for a table with enums", async () => {
      ...
      try {
         ...
         const formattingIssues = Object.fromEntries(Object.entries(files).flatMap(([name, contents]) => {
            const issues = [];
            if (contents.split("\n").some((line) => /\s+$/.test(line))) issues.push("trailing whitespace");
            if (contents.endsWith("\n\n")) issues.push("extra final newline");
            return issues.length ? [[name, issues]] : [];
         }));
         expect(formattingIssues).toMatchInlineSnapshot(`{}`);
