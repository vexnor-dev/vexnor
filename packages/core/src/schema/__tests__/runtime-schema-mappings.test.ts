...
function column(table_schema: string, table_name: string, column_name: string, options: Partial<SqlColumnInfo> = {}): SqlColumnInfo {
   return {
      table_schema,
      table_name,
      column_name,
      ...
      tables: [
         {
            ...
            columns: [
               ...
               column("alpha", "record", "created_at", { ordinal_position: 3, udt_name: "datetime", column_default: "current_timestamp" }),
            ],
            primary_keys: [
               { table_schema: "alpha", table_name: "record", constraint_name: "record_pk", column_name: "tenant_id", ordinal_position: 1 },
               { table_schema: "alpha", table_name: "record", constraint_name: "record_pk", column_name: "record_id", ordinal_position: 2 },
            ],
            foreign_keys: [],
         },
         {
            ...
            primary_keys: [],
            foreign_keys: [
               { table_schema: "beta", table_name: "event_log", constraint_name: "event_record_fk", column_name: "tenant_id", referenced_table_schema: "alpha", referenced_table_name: "record", referenced_column_name: "tenant_id", ordinal_position: 1 },
               { table_schema: "beta", table_name: "event_log", constraint_name: "event_record_fk", column_name: "record_id", referenced_table_schema: "alpha", referenced_table_name: "record", referenced_column_name: "record_id", ordinal_position: 2 },
            ],
         },
         {
            table_schema: "beta",
...
describe("createRuntimeSchemaMappings", () => {
   test("creates selected mappings for PK, PK-less, and view objects with composite relationships", async () => {
      ...
      const selection = await resolveSchemaSelection({ catalog: currentCatalog, request: { mode: "non-interactive", all: true } });
      ...
      expect({
      ...
      }).toMatchInlineSnapshot(`
        {
          ...
          "source": "vexnor-local:26507fd8610e398cd9e34096783d093ac41458d506e285ff969dfc0e5596134d",
        }
      `);
   });
   ...
   test("uses qualified identities as keys when generated mapping names collide", async () => {
      ...
      expect(Object.keys(createRuntimeSchemaMappings({ catalog: currentCatalog, selection: selection.scope }).schema)).toMatchInlineSnapshot(`
        [
          "alpha.record",
          "beta.record",
        ]
   ...
   test("omits unresolved UDT values while preserving catalog-backed enum values", async () => {
      ...
      const table = createRuntimeSchemaMappings({ catalog: currentCatalog, selection: selection.scope }).mappings[0]!.table;
