...
import type {
   ...
   SqlColumnType,
   SqlColumnTypeTree,
   SqlColumnTypeTreeField,
   SqlForeignKeyInfo,
   SqlSchema,
} from "#src/plugin/plugin.js";
...
export interface SchemaCatalogColumn {
   ...
   array: boolean;
   typeTree: SchemaCatalogColumnTypeTree | null;
...
export type SchemaCatalogColumnTypeTree =
   | { kind: "scalar"; type: SqlLiteralType; udt: string | null }
   | { kind: "struct"; fields: SchemaCatalogColumnTypeTreeField[] }
   | { kind: "list"; value: SchemaCatalogColumnTypeTree; length: number | null }
   | { kind: "map"; key: SchemaCatalogColumnTypeTree; value: SchemaCatalogColumnTypeTree }
   | { kind: "union"; members: SchemaCatalogColumnTypeTreeField[] };

export interface SchemaCatalogColumnTypeTreeField {
   physicalName: string;
   mappingName: string;
   value: SchemaCatalogColumnTypeTree;
}
...
export function createSchemaCatalog({ plugin, schema, naming }: CreateSchemaCatalogOptions): SchemaCatalog {
         ...
         const columns = table.columns
            .map((column, index): SchemaCatalogColumn => {
               ...
               return {
                  ...
                  generated:
                     column.is_generated !== undefined &&
                     column.is_generated !== "NEVER" &&
                     column.is_generated !== "NO",
                  ...
                  typeTree: columnType.typeTree
                     ? catalogTypeTree(columnType.typeTree, naming?.camelCaseColumns ?? false)
                     : null,
            ...
            .sort(
               (left, right) =>
                  left.ordinalPosition - right.ordinalPosition || left.physicalName.localeCompare(right.physicalName),
            );
   ...
   const joinableObjects = new Set(
      catalogObjects.flatMap((object) =>
         object.relationships.flatMap((relationship) => [relationship.fromObject, relationship.toObject]),
      ),
   );
   ...
   const pluginWarnings: SchemaCatalogWarning[] =
...
function catalogTypeTree(typeTree: SqlColumnTypeTree, camelCaseColumns: boolean): SchemaCatalogColumnTypeTree {
   switch (typeTree.kind) {
      case "scalar":
         return { kind: "scalar", type: typeTree.type, udt: typeTree.udt ?? null };
      case "struct":
         return { kind: "struct", fields: catalogTypeTreeFields(typeTree.fields, camelCaseColumns) };
      case "list":
         return {
            kind: "list",
            value: catalogTypeTree(typeTree.value, camelCaseColumns),
            length: typeTree.length ?? null,
         };
      case "map":
         return {
            kind: "map",
            key: catalogTypeTree(typeTree.key, camelCaseColumns),
            value: catalogTypeTree(typeTree.value, camelCaseColumns),
         };
      case "union":
         return { kind: "union", members: catalogTypeTreeFields(typeTree.members, camelCaseColumns) };
   }
}

function catalogTypeTreeFields(
   fields: SqlColumnTypeTreeField[],
   camelCaseColumns: boolean,
): SchemaCatalogColumnTypeTreeField[] {
   return fields.map((field) => ({
      physicalName: field.name,
      mappingName: camelCaseColumns ? to.camel(field.name) : field.name,
      value: catalogTypeTree(field.value, camelCaseColumns),
   }));
}
...
function createRelationships(fromObject: string, foreignKeys: SqlForeignKeyInfo[]): SchemaCatalogRelationship[] {
   const groups = new Map<
      string,
      { constraintName: string; toObject: string; pairs: { from: string; to: string; ordinal: number }[] }
   >();
...
function assertUniqueObjectIds(schema: SqlSchema): void {
   assertUnique(
      "schema object",
      schema.tables.map((table) => qualify(table.table_schema, table.table_name)),
   );
   assertUnique(
      "schema enum",
      schema.enums.map((entry) => qualify(entry.enum_schema, entry.enum_name)),
   );
}
...
function compareOrdinalThen<T extends { ordinal: number }>(
