...
function AccountGrid({
   ...
   function onDelete(accountId: string) {
      deleteAccount.sqlite.run({ db: remoteClient, params: { accountId } }).then(() => startTransition(onRefresh));
   }
...
export default function Sqlite3AccountsPage() {
   const [promise, setPromise] = useState(() => selectAccounts.sqlite.all({ db: remoteClient }));

   function refresh() {
      setPromise(selectAccounts.sqlite.all({ db: remoteClient }));
   }
