...
import type { IAccountSelect } from "#shared/codegen/postgres/vexnor_dev.account-table.js";
import type { IOrderSelect } from "#shared/codegen/postgres/vexnor_dev.order-table.js";
...
export default function PostgresLoginPage() {
   const { login } = useAuth("postgres");
   const navigate = useNavigate();
   const [promise, setPromise] = useState<Promise<LoginAccount[]>>(Promise.resolve([]));

   useEffect(() => {
      setPromise(selectAccountsForLogin.postgres.all({ db: remoteClient }) as Promise<LoginAccount[]>);
   }, []);

   function handlePick(account: LoginAccount) {
      login(makeFakeToken(account.accountId, `${account.firstName} ${account.lastName}`));
      void navigate({ to: "/postgres", search: { filter: undefined } });
   }

   return (
      <div className="page">
         <h1>Sign in — PostgreSQL</h1>
