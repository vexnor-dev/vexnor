...
import { InferWindowByRow } from "@vexnor/core";
...
describe("Account.postgres.select() — windowBy SQL generation", () => {
   // --- Ranking (5) ---
...
describe("Account.postgres.select() — windowBy result type inference", () => {
   test("result includes window alias as typed field", () => {
      type Params = { windowBy: { myRank: { fn: "rank"; over: { orderBy: { createdAt: "ASC" } } } } };
      type Inferred = InferWindowByRow<{ Select: IAccountSelect }, Params>;

      // myRank is on the inferred type
      const _rank: Inferred["myRank"] = 1 as number;
      void _rank;
   });

   test("ranking fn returns number (never null)", () => {
      type Params = { windowBy: { rowNum: { fn: "row_number"; over: { orderBy: { createdAt: "ASC" } } } } };
      type Inferred = InferWindowByRow<{ Select: IAccountSelect }, Params>;

      // InferWindowByRow should produce { rowNum: number }
      const _val: Inferred["rowNum"] = 1 as number;
      void _val;
   });

   test("lag returns column type | null", () => {
      type Params = { windowBy: { prevEmail: { fn: "lag"; col: "email"; args: 1; over: { orderBy: { createdAt: "ASC" } } } } };
      type Inferred = InferWindowByRow<{ Select: IAccountSelect }, Params>;

      // lag(email) → string | null
      const _val: Inferred["prevEmail"] = "test" as string | null;
      void _val;
      const _null: Inferred["prevEmail"] = null;
      void _null;
   });

   test("sum returns number | null", () => {
      type Params = { windowBy: { total: { fn: "sum"; col: "createdAt"; over: { orderBy: { createdAt: "ASC" } } } } };
      type Inferred = InferWindowByRow<{ Select: IAccountSelect }, Params>;

      // sum → number | null
      const _val: Inferred["total"] = 1 as number | null;
      void _val;
      const _null: Inferred["total"] = null;
      void _null;
   });

   test("non-existent window alias is not on inferred type", () => {
      type Params = { windowBy: { myRank: { fn: "rank"; over: { orderBy: { createdAt: "ASC" } } } } };
      type Inferred = InferWindowByRow<{ Select: IAccountSelect }, Params>;

      // Only "myRank" exists — verify by assignment
      const _myRank: Inferred["myRank"] = 1;
      void _myRank;

      // @ts-expect-error — 'notAField' does not exist on inferred window result
      type _Bad = Inferred["notAField"];
   });

   test("without windowBy — InferWindowByRow resolves to unknown", () => {
      type Params = {};
      type Inferred = InferWindowByRow<{ Select: IAccountSelect }, Params>;

      // Should be unknown — no augmentation
      const _val: Inferred = undefined as unknown;
      void _val;
   });
});
