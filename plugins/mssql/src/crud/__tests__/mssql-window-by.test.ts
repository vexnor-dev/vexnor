...
function buildWindowBy(windowBy: WindowBySelect<{ Select: IAccountSelect }>) {
   const query = sqlSelect(Account, {}, info({ driver: "transactsql" }));
   return query.getSql({
      params: { windowBy },
...
describe("Account.mssql.select() — windowBy SQL generation", () => {
   // --- Ranking (5) ---
   ...
   test("MSSQL — RANGE with numeric bounds throws", () => {
      expect(() =>
         buildWindowBy({ total: { fn: "sum", col: "createdAt", over: { orderBy: { createdAt: "ASC" }, frame: "range", start: 3, end: 0 } } }),
      ).toThrow(/MSSQL does not support numeric bounds with RANGE frame/);
   });
});
