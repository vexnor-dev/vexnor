...
export class TestDataManager {
   ...
   async cleanAll(db: Database.Database) {
      ...
      await sql`delete from ${OrderItem} where ${OrderItem.$createdAt} < ${timestamp}`.sqlite3.run({ db });
      await sql`delete from ${Order} where ${Order.$createdAt} < ${timestamp}`.sqlite3.run({ db });
      await sql`delete from ${Product}`.sqlite3.run({ db });
      await sql`delete from ${Account} where ${Account.$createdAt} < ${timestamp}`.sqlite3.run({ db });
