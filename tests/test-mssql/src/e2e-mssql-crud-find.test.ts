...
describe.sequential("vexnor mssql table handler - find", async (ctx) => {
   ...
   test("findById: fetch account by PK", async () => {
      const result = await Account.mssql.findById().any({ db: pool.request(), params: { accountId: rootAccount.accountId } });
   ...
   test("findBy: fetch account by email", async () => {
      const result = await Account.mssql.findBy().any({ db: pool.request(), params: { email: rootAccount.email } });
      expect(result?.accountId).toBe(rootAccount.accountId);
   });
   ...
   test("findBy: fetch account by multiple fields", async () => {
      const result = await Account.mssql
         .findBy()
         .any({ db: pool.request(), params: { email: rootAccount.email, lastName: rootAccount.lastName } });
      expect(result?.accountId).toBe(rootAccount.accountId);
   });
});
